...
def make_chart(df_dict: dict,
    ...
    """
    A function to generate charts for the selected pollutants and stations.
    Each time this function is called it will generate a series of charts for the specified pollutant
    :param df_dict: dictionary of data to chart - keys = pollutants values = dataframe of pollutant concentrations for selected sites
    :param pollutant: pollutant to chart
    :param avg_time: averaging period for charting pollutant
    :param start: user input start date for slicing data from dataframe
    :param end: user input end date for slicing data from dataframe
    :param output_figure_name: name for output figure
    :param show_criteria: option to show criteria line on chart
    :param y_axis_limit: limits for y axis
    :param show_average: option to show average line on chart - Default is False
    :param no_nepm: option to ignore NEPM requirement of 75% data capture - default is False
    :param criteria_label: Custom label for "criteria" - default is "Objective"
    :param custom_criteria: Custom criteria value
    :param state: state for criteria values
    :param station_file_info: dataframe containing information on the stations
    :param font: Default is "arial"
    :param x_frmt: Optional x-axisdate label string format
    :param comp_csv: dataframe containing pollutant data - empty dataframe if not selected
    :param comp_lab: label for comparison csv data
    :param comp_chart_type: optional type for comparison data chart - default is "bar"
    :param cust_y_label: Custom y label name
    :return:
    """
    # chart_font = 'Arial'
    ...
    if custom_criteria is not None:
        ...
        import math
        chart_criteria[pollutant_criteria] = math.ceil(pollutant_criteria * 1.3)
    else:
    ...
    if chart_data.isnull().sum().sum() == chart_data.shape[0] * chart_data.shape[1]:
        ...
        if y_axis_limit > 0:
            ...
            if show_criteria:
                try:
                    ...
                    print(f"\n********** ERROR ********** If using a custom pollutant/ averaging period with a "
                          f"criteria line you must enter a custom criteria value\n")
                    raise TypeError
            else:
                y_max = (max_conc // 10 + 1) * 10
        ...
        if cust_colours is not None:
            colours = cust_colours
        ...
        if avg_time != 'Y':
            ...
            for x in range(len(years)):
                ...
                for site in sites:
                    ...
                    ax.plot(mdates.date2num(list(year_data.index)), year_data.iloc[:,i], label=label, linewidth=linewidth)  # c=colours[i]
                    if show_average:
                ...
                if show_average:
                    ...
                    # if comp_csv.shape[0] != 0:
                    #     leg2 = bar_ax.legend(fontsize=font - ((font-8) / 2), loc="upper right")
                ax.set_ylabel(pollutant_lab, fontsize=font) # fontname=chart_font,
...
def background_data_charts(stations: str,
                           ...
                           **kwargs):
    """
    Main function to generate the pollutant charts
    :param stations: list of stations from GUI in string form
    ...
    if data_dict_empty(data_dict):
        ...
        # get custom colours
        cust_colours = kwargs.get("custom_colours", None)
        if cust_colours is not None:
            cust_colours = cust_colours.split(",")
            cust_colours = [x.strip() for x in cust_colours]
        ...
        for plt, avg in zip(pollutants, avg_prd_list):
            ...
            if plt == 'CO (ug/m^3)' and avg == '1H' and state == 'qld':
                ...
                make_chart(data_dict, plt, avg, start, end, chart_name, False,
                           ...
                           station_file_info, font, x_form, comp_data, comp_label, comp_type, cust_poll_label, cust_colours)
