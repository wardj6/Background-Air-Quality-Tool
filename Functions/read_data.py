...
def get_stations_and_files(dir: str) -> pd.DataFrame:
    """
    Read list of available stations from relevant database directory
    :param dir: location of database and station list file
    :return: dataframe containing station list csv data
    """
...
def get_stations(stations: str) -> list:
    """
    Generate list of stations from string entered in GUI
    :param stations:
...
def get_state(station_info_df: pd.DataFrame,
    ...
    """
    A function to find the state for entered stations - for identifying the correct criteria values
    :param station_info_df:
    :param custom_state: Custom state string entered in GUI - default is None
    :return: The state (in string form) that the stations are located in
    """
...
def get_station_info(dirs: list,
    ...
    """
    A function to get information for each stations that will be used in the background tool
    This will return an error if the stations are in different states - criteria values differ between regulatory jurisdictions
    :param dirs: list of directories to search for the selectd stations
    :param stations: list of selected stations
    :return: Dataframe containing station name (as per GUI input), file path and chart name (label for chart legend) for each station
    """
...
def get_data(r_df: pd.DataFrame,
    ...
    """
    This function slices the dataframe by start and end time and adds empty columns for missing pollutants
    :param r_df: raw dataframe to parse
    :param start: start date
    :param end: end date
    :param my_pollutants: list of pollutants required in dataframe
    :param missing_pollutants: list of pollutants missing from dataframe
    :return: sliced dataframe containing new columns for any missing pollutants
    """
...
def get_pollutant_dataframes(stations: str,
    ...
    """
    This function reads in data from the appropriate databases
    :param stations: list of stations from GUI in string form
    :param plts_to_parse: list of pollutants to read data for
    :param start_date: start date for data
    :param end_date: end date for data
    :param kwargs:
    :return: dictionary keys = pollutant column name, values = pollutant timeseries data for each station selected
    """
...
def data_dict_empty(data_dict: dict) -> bool:
    """
    Determines whether the data dictionary is empty - for error checking purposes
    :param data_dict:
